...
const CustomizeProducts = ({ productId, variants, productOptions }: { productId: string, variants: products.Variant[], productOptions: products.ProductOption[] }) => {
  ...
  return (
    <div className='flex flex-col gap-6'>
          ...
          <ul className='flex items-center gap-3'>
            {option.choices?.map((choice) => {
              ...
              return option.name === 'Color' ? (
                <li 
                  className='w-8 h-8 rounded-full ring-1 ring-gray-500 relative' 
                ...
                <li className='ring-1 ring-lama text-lama rounded-md py-1 px-4 text-sm cursor-pointer' style={{
                  cursor:disabled ? "not-allowed" : "pointer",
                  background: selected ? "#f35c7a" : disabled ? "#FBCFE8" :"white",
      ...
      {/* <h4 className='font-medium'>Choose a {option.name}</h4>
        <ul className='flex items-center gap-3'>
          <li className='w-8 h-8 rounded-full ring-1 ring-gray-500 cursor-pointer relative bg-red-500'>
            <div className='absolute w-10 h-10 rounded-full ring-2 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2'></div>
          </li>
          <li className='w-8 h-8 rounded-full ring-1 ring-gray-500 cursor-pointer relative bg-blue-500'>
          </li>
      ...
      <ul className='flex items-center gap-3'>
        ...
        <li className='ring-1 ring-pink-200 text-white rounded-md py-1 px-4 bg-pink-200 text-sm cursor-not-allowed'>
          Large
        </li>
      </ul> */}
    </div >
